...
const updateSearchValue = (fixture: ComponentFixture<TestSelectComponent>, searchValue: string) => {
...
describe('SelectComponent', () => {
  ...
  it('Search', async () => {
    ...
    const getOptionTexts = async () => {
      const options = await select.getOptions(SELECTOR_ITEM_OPTION);
    ...
    let texts = await getOptionTexts();
    ...
    expect(texts).toEqual(['AAA AAA', 'AAA BBB', 'BBB CCC']);
    /*
    searchOptions[0].host()
      .then((node) => node.getAttribute('class'))
      .then(console.log);
    const hasInput = await searchOptions[0].hasHarness(MatInputHarness);
