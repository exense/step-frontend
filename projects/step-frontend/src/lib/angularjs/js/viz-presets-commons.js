...
function TimelineWidget(outerScope) {
  ...
  var textFilters =
    '[{ "key": "eId", "value": "__businessobjectid__", "regex": "false" }, { "key": "type", "value": "__measurementType__", "regex": "false" }]';
  ...
  var timelineWidget = new Widget(
    ...
    new DashletState(
      ...
      wOptions,
      config,
      new RTMAggBaseTemplatedQueryTmpl(
        'cnt',
        'auto',
        overtimeFillBlanksTransformFn.toString(),
        entityName,
        timeField,
        timeFormat,
        valueField,
      ...
      $(document).ready(function () {
        if (!outerScope.timelineReady) {
          ...
          if (!outerScope.listenerSetup) {
            console.log(
              'timeline data was loaded after chart was configured: listener setup by timeline data complete.'
            );
...
function WikimediaDemo() {
  ...
  var wikimediaQueryTemplate = new TemplatedQuery(
    ...
    new Controls(
      new Template(
        '',
        'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/all-agents/Foo/daily/__dayFrom__/__dayTo__',
  ...
  var widget = new Widget(
    getUniqueId(),
    new WidgetState('col-md-12', false, true),
    new DashletState(
      ' Daily wikimedia stats',
      false,
      0,
      {},
      options,
      new Config('Fire', 'Off', false, false, ''),
      wikimediaQueryTemplate,
      new DefaultGuiClosed(),
      new DefaultInfo()
    )
  );
  ...
  var dashboardObject = new Dashboard(
    'Daily Stats',
    new DashboardState(
      new GlobalSettings([new Placeholder('__businessobjectid__', '', false)], false, false, 'Global Settings', 3000),
      widgetsArray,
      'Wikimedia Dashboard',
      'aggregated',
...
function RTMAggBaseQueryTmpl(metric, transform) {
  return new AsyncQuery(
    ...
    new Service( //service
      ...
      '', //templated
      new Preproc(
        'function(requestFragment, workData){var newRequestFragment = requestFragment;for(i=0;i<workData.length;i++){newRequestFragment = newRequestFragment.replace(workData[i].key, workData[i].value);}return newRequestFragment;}'
      ),
      new Postproc(
        '',
        '',
        [],
        "function(response){if(!response.data.payload){console.log('No payload ->' + JSON.stringify(response)); return null;}return [{ placeholder : '__streamedSessionId__', value : response.data.payload.streamedSessionId, isDynamic : false }];}",
        ''
      )
    ),
    new Service( //callback
      ...
      new Preproc(
        'function(requestFragment, workData){var newRequestFragment = requestFragment;for(i=0;i<workData.length;i++){newRequestFragment = newRequestFragment.replace(workData[i].placeholder, workData[i].value);}return newRequestFragment;}'
      ),
      new Postproc(
        'function(response){return response.data.payload.stream.complete;}',
        transform,
...
function RTMAggBaseTemplatedQueryTmpl(
  metric,
  pGranularity,
  transform,
  entityName,
    ...
    new Controls(
      new Template(
        '{ "selectors1": [{ "textFilters": ' +
          ...
          ', "numericalFilters": ' +
          numericalFilters +
          ' }], "serviceParams": { "measurementService.nextFactor": "0", "aggregateService.timeField" : "' +
          timeField +
          '", "aggregateService.timeFormat" : "' +
          timeFormat +
          '", "aggregateService.valueField" : "' +
          valueField +
          '", "aggregateService.sessionId": "defaultSid", "aggregateService.granularity": "__granularity__", "aggregateService.groupby": "' +
          groupby +
          '", "aggregateService.cpu": "1", "aggregateService.partition": "1", "aggregateService.timeout": "600" } }',
        '',
        [
...
function RTMAggTimeFrameTemplatedQueryTmpl(
  metric,
  pGranularity,
  transform,
  entityName,
...
var addAggregatesSummaryTpl = function (
  widgetsArray,
  entityName,
  timeField,
  timeFormat,
  ...
  var standalone = new Widget(
    getUniqueId(),
    new WidgetState('col-md-6', false, true),
    new DashletState(
      entityName + ' stats summary',
      false,
      0,
      {},
      new EffectiveChartOptions('seriesTable', 'function(d) { return d; }', timeFrame),
      new Config('Fire', 'Off', false, false, '', 3000, 1000, 'Off', 8, 'On'),
      new RTMAggBaseTemplatedQueryTmpl(
        'sum',
        'max',
        summaryTransform,
...
var addAggregatesOverTimeTpl = function (
  widgetsArray,
  entityName,
  timeField,
  timeFormat,
  ...
  var overtimeTransform =
    'function (response, args) {\r\n    var metric = args.metric;\r\n    var retData = [], series = {};\r\n\r\n    var payload = response.data.payload.stream.streamData;\r\n    var payloadKeys = Object.keys(payload);\r\n\r\n    for (i = 0; i < payloadKeys.length; i++) {\r\n        var serieskeys = Object.keys(payload[payloadKeys[i]])\r\n        for (j = 0; j < serieskeys.length; j++) {\r\n            retData.push({\r\n                x: payloadKeys[i],\r\n                y: payload[payloadKeys[i]][serieskeys[j]][metric],\r\n                z: serieskeys[j]\r\n            });\r\n        }\r\n    }\r\n    return retData;\r\n}';
  var config = getMasterSlaveConfig(
    'raw',
    'Average ' + entityName + ' Duration (ms)',
    'Nb ' + entityName + 's per second'
  );
  ...
  var master = new Widget(
    ...
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('lineChart', null, timeFrame, __from__, __to__),
      config.masterconfig,
      new RTMAggBaseTemplatedQueryTmpl(
        'avg',
        'auto',
  ...
  var slave = new Widget(
    ...
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('stackedAreaChart', null, timeFrame, __from__, __to__),
      config.slaveconfig,
      new RTMAggBaseTemplatedQueryTmpl(
        'tps',
        'auto',
...
var addLastMeasurementsTpl = function (
  widgetsArray,
  entityName,
  timeField,
  timeFormat,
      ...
      new Controls(
        new Template(
          '{ "selectors1": [{ "textFilters": ' +
            ...
            ', "numericalFilters": ' +
            numericalFilters +
            ' }], "serviceParams": { "measurementService.nextFactor": "__FACTOR__", "aggregateService.timeField" : "' +
            timeField +
            '", "aggregateService.timeFormat" : "' +
            timeFormat +
            '", "aggregateService.valueField" : "' +
            valueField +
            '", "aggregateService.sessionId": "defaultSid", "aggregateService.granularity": "auto", "aggregateService.groupby": "' +
            groupby +
            '", "aggregateService.cpu": "1", "aggregateService.partition": "1", "aggregateService.timeout": "600" } }',
          '',
  ...
  var config = getMasterSlaveConfig(
    'transformed',
    'Last 100 ' + entityName + 's - Individual duration (ms)',
    'Last 100 ' + entityName + 's - Value table (ms)'
  );
  ...
  var master = new Widget(
    ...
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('scatterChart', null, null),
      config.masterconfig,
      new RTMLatestMeasurementTemplatedQuery(
        entityName,
        timeField,
        timeFormat,
        valueField,
  ...
  var slave = new Widget(
    ...
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('seriesTable', null, null),
      config.slaveconfig,
      new RTMLatestMeasurementTemplatedQuery(
        entityName,
        timeField,
...
var addLastMeasurements = function (
  widgetsArray,
  entityName,
  timeField,
  timeFormat,
    ...
    return new SimpleQuery(
      ...
      new Service(
        ...
        '{"selectors1": [{ "textFilters": ' +
          ...
          ', "numericalFilters": ' +
          numericalFilters +
          ' }],"serviceParams": { "measurementService.nextFactor": "100", "aggregateService.timeField" : "' +
          timeField +
          '", "aggregateService.timeFormat" : "' +
          timeFormat +
          '", "aggregateService.valueField" : "' +
  ...
  var config = getMasterSlaveConfig(
    'raw',
    'Last 100 Measurements - Scattered values (ms)',
    'Last 100 Measurements - Value table (ms)'
  );
  var master = new Widget(
    config.masterid,
    new DefaultWidgetState(),
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('scatterChart', null, timeFrame, __from__, __to__),
      config.masterconfig,
  ...
  var slave = new Widget(
    ...
    new DashletState(
      ...
      0,
      {},
      new EffectiveChartOptions('seriesTable', null, timeFrame, __from__, __to__),
      config.slaveconfig,
      new RTMLatestMeasurementBaseQuery(),
      new DefaultGuiClosed(),
      new DefaultInfo()
...
var addErrorsSummary = function (
  ...
  var standalone = new Widget(
    ...
    new DefaultWidgetState(),
    new DashletState(
      entityName + ' status summary',
      false,
      0,
      {},
      new EffectiveChartOptions('singleValueTable', 'function(d) { return d; }', timeFrame, __from__, __to__),
      new Config('Fire', 'Off', false, false, ''),
      new RTMAggBaseTemplatedQueryTmpl(
        'cnt',
        'max',
...
var addErrorsOverTimeTpl = function (
  widgetsArray,
  entityName,
  timeField,
  timeFormat,
