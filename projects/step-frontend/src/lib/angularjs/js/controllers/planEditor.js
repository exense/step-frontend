  ...
  .controller(
    ...
    function (
      ...
      $scope.exportPlan = function () {
        ExportDialogs.displayExportDialog(
          'Plans export',
          'plans/' + $scope.planId,
      ...
      $scope.clonePlan = function () {
        modalResult = Dialogs.enterValue(
          'Clone plan as ',
          $scope.model.plan.attributes.name + '_Copy',
          'md',
          'enterValueDialog',
          function (value) {
      ...
      //------------------------------------
      $scope.artefactRef = function () {
        return { repositoryID: 'local', repositoryParameters: { planid: $scope.planId } };
      };
        ...
        execute: function (artefacts) {
          ...
          function execute(artefacts, callback) {
            if (artefacts.length > 0) {
              ...
              $http
                .post('rest/interactive/' + sessionId + '/execute/' + $scope.model.plan.id + '/' + artefact.id)
                .then(function () {
      ...
      $scope.startInteractive = function () {
        var executionParameters = {
          repositoryObject: $scope.artefactRef,
          userID: '',
          mode: 'RUN',
          customParameters: $scope.executionParameters,
        };
