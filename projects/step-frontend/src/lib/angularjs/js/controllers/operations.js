 ...
 *******************************************************************************/
angular
  .module('operationsControllers', ['step'])

  .run(function (ViewRegistry, EntityRegistry) {
    ViewRegistry.registerView('operations', 'partials/operations/operationsList.html');
    ViewRegistry.registerMenuEntry('Current Operations', 'operations', 'glyphicon glyphicon-eye-open',  10,"status-root", 'operations-read');
  })

  .controller(
    'OperationListCtrl',
    function ($rootScope, $scope, $http, stateStorage, Dialogs, ResourceDialogs, AuthService) {
      ...
      $scope.loadOperationsData = function () {
        $http.get('rest/threadmanager/operations/list').then(function (response) {
