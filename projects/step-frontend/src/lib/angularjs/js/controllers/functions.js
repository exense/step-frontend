  ...
  .controller(
    ...
    function (
      ...
      $scope.initCtrl = function (type) {
        if (type == 'masks') {
          $scope.config = FunctionDialogsConfig.getConfigObject(
            'Mask',
            'masks',
      ...
      $scope.duplicateFunction = function (id) {
        $http.post('rest/' + $scope.config.serviceRoot + '/' + id + '/copy').then(() => reload());
      };
      ...
      $scope.addFunction = function () {
        FunctionDialogs.addFunction(function () {
          reload();
        }, $scope.config);
      };
      ...
      $scope.deleteFunction = function (id, name) {
        Dialogs.showDeleteWarning(1, 'element "' + name + '"').then(function () {
          $http.delete('rest/' + $scope.config.serviceRoot + '/' + id).then(function () {
            reload();
          });
        });
      };
      ...
      _.each(_.where(columns, { title: 'Actions' }), function (col) {
        ...
        col.render = function (data, type, row) {
          var html =
            ...
            '<div class="btn-group">' +
            '<button type="button" class="btn btn-default" aria-label="Left Align" onclick="angular.element(\'#FunctionListCtrl\').scope().selectFunction(\'' +
            row[0] +
            '\')">' +
    ...
    return {
      ...
      controller: function ($scope, FunctionDialogs, FunctionDialogsConfig, LinkProcessor) {
        ...
        $scope.openFunctionEditor = function () {
          FunctionDialogs.openFunctionEditor(
            $scope.function_.id,
            FunctionDialogsConfig.getConfigObject('Keyword', 'functions', [], false, 'functionTable')
          );
        };
        $scope.openLink = () =>
