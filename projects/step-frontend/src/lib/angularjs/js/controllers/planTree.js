 ...
 *******************************************************************************/
angular
  .module('planTree', ['step', 'artefacts', 'reportTable', 'dynamicForms', 'export', 'functionsControllers'])
        ...
        controller: function ($scope, $location, $rootScope, AuthService, FunctionDialogs) {
                      ...
                      if (function_.schema && function_.schema.properties) {
                        ...
                        if (function_.schema.required) {
                          _.each(function_.schema.required, function (prop) {
                            if (targetObject[prop] && targetObject[prop].value) targetObject[prop].value += ' (REQ)';
          ...
          $scope.openSelectedArtefact = function () {
            ...
            if (selectedArtefact.original.planId) {
              $http
                .get('rest/plans/' + $scope.plan.id + '/artefacts/' + selectedArtefact.id + '/lookup/plan')
          ...
          function openPlan(planId) {
            $http.get('rest/plans/' + planId).then(function (response) {
              LinkProcessor.process(response.data.attributes.project)
                .then(() => {
          ...
          function sortNodeListByPositionInTree(nodeList) {
            var flatNodeIdList = _.map(buildFlatNodeList($scope.plan.root), function (node) {
              return node.id;
            });
          ...
          $scope.move = function (offset) {
             ...
             */
                ...
                if (newIndex >= 0 && newIndex < children.length) {
                  ...
                  nodeIdsToBeMoved = _.reject(nodeIdsToBeMoved, function (id) {
                    return id == node.id;
                  });
                }
              });
