      ...
      function createHeaderRenderer(headerTransclude) {
        return function (element, column, handle) {
          ...
          var content = headerTransclude(function (clone, scope) {
            ...
            headerScope.search = function (expression) {
              handle.search(colDef.name, expression);
            };
          });
          element.empty();
          ...
          if (columns && columns.length > 0) {
            ...
            if (scope.persistState == null || scope.persistState) {
              if (scope.uid) {
                ...
                console.error(
                  "Unable to persist table state if the table uid isn't specified. Please set the attribute 'uid'"
                );
              }
            }
              ...
              tableOptions.ajax = {
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                  ...
                  if (jqXHR.status === 500 && jqXHR.responseText.indexOf('MongoExecutionTimeoutException') >= 0) {
                    Dialogs.showErrorMsg(
                      '<strong>Timeout expired.</strong><Br/>The timeout period elapsed prior to completion of the DB query.'
                    );
                  }
                },
              };
            ...
            table
              ...
              .each(function (i) {
                table
                  .settings()[0]
                  .aoColumns[i].headerRenderer(
                    angular.element(table.column(i).header()),
                    table.column(i),
                    scope.handle
                  );
              });
            ...
            $('th', tableElement.find('thead>tr:not(.searchheader)').eq(0))
              .css({ 'border-bottom': '0' })
...
window.alert = (function () {
  ...
  return function (message) {
    if (message) {
      message.toString().indexOf('DataTables warning') === 0 ? console.warn(message) : nativeAlert(message);
    } else {
      nativeAlert(message);
    }
  };
