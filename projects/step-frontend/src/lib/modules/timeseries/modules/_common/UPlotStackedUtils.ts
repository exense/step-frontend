// @ts-nocheck
// This is an utility file created by the creators of uPlot. Stacked series is not part of uPlot, so this is needed in addition.
declare const uPlot: any;

export class UPlotStackedUtils {
  public static stack(data, omit) {
    let data2 = [];
    let bands = [];
  ...
  public static getOpts(title, series) {
    return {
      scales: {
        x: {
          time: false,
  ...
  public static getStackedOpts(title, series, data, interp) {
    let opts = this.getOpts(title, series);

    let interped = interp ? interp(data) : data;

    let stacked = this.stack(interped, (i) => false);
