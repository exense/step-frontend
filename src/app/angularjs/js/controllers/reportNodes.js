...
angular
  ...
  .factory('ReportNodeCommons', function (ScreenTemplates) {
    ...
    ScreenTemplates.getScreenInputsByScreenId('functionTable').then(function (attributes) {
      functionAttributes = _.sortBy(attributes, function (value) {
        return value.id;
      });
    });
      ...
      controller: function ($scope, $http, Dialogs, LinkProcessor, $location) {
        var reportNodeTypes = {
          'step.artefacts.reports.CallFunctionReportNode': {
            template: 'partials/reportnodes/callFunctionReportNode.html',
          },
        ...
        $scope.$watch('id', function (nodeId) {
          if (nodeId) {
            $http.get('rest/controller/reportnode/' + nodeId).then(function (response) {
              ...
              $scope.node = node;
              $scope.reportNodeType =
                node._class in reportNodeTypes ? reportNodeTypes[node._class] : reportNodeTypes['Default'];
