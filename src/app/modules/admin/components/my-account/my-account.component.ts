...
import { MyAccountDto } from '../../shared/my-account-dto';
import { MyAccountPreferencesDto } from '../../shared/my-account-preferences-dto';
...
export class MyAccountComponent implements OnInit, OnChanges {
    ...
    savePreferences(): void {
        ...
        this._http
            .post('rest/admin/myaccount/preferences', preferences)
            .subscribe({
                error: err => {
    ...
    ngOnInit(): void {
        const response$ = this._http.get("rest/admin/myaccount") as Observable<HttpResponse<Partial<MyAccountDto>>>;
        response$.pipe(filter(r => r.ok)).subscribe(r => {
            this.user = r.body || {};    
...
.directive(
    'stepMyAccount',
    downgradeComponent({component: MyAccountComponent})
);